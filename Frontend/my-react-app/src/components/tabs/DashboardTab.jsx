...
import { todayStr, formatDate, isSameStudent } from "../../store/useAttendanceStore";
...
function DashboardTab({ store, currentDate, currentTime, setActiveTab }) {
  ...
  const getWeeklyData = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayRecords = attendanceRecords.filter((a) => a && (a.date || a.attendance_date) === dateStr);
      const present = dayRecords.filter((a) => a && a.status && String(a.status).toLowerCase() === "present").length;
  ...
  const getClassData = () => {
    ...
    return grades.map((grade) => {
      ...
      const todayClassRecords = attendanceRecords.filter(
        (a) => a && (a.date || a.attendance_date) === today && classStudents.some((s) => isSameStudent(s, a))
      );
      ...
      <section className="metrics-grid">
        <div className="metric-card bg-glass">
          ...
          <div className="metric-info">
            <h3>{todayStats.total || 0}</h3>
            <p>Total Enrolled</p>
            <span className="metric-trend text-blue">Students registered</span>
          </div>
        </div>
        ...
        <div className="metric-card bg-glass">
          ...
          <div className="metric-info">
            <h3>{todayStats.presentToday || 0}</h3>
            <p>Present Today</p>
            <span className="metric-trend text-green">
              {todayStats.hasToday ? "Marked" : "Not marked yet"}
            </span>
          </div>
        </div>
        ...
        <div className="metric-card bg-glass">
          ...
          <div className="metric-info">
            <h3>{todayStats.absentToday || 0}</h3>
            <p>Absent Today</p>
                      ...
                      <td>
                        <div className="student-profile">
                          <div className="avatar-badge">
                            {record.student?.name ? record.student.name.split(" ").map((n) => n[0]).join("").slice(0, 2) : "S"}
                          </div>
